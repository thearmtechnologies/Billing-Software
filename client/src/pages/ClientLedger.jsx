      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.title}>
            {clientInfo?.name || "Client Ledger"}
          </Text>
          {clientInfo?.phone && (
            <Text style={styles.subtitle}>{clientInfo.phone}</Text>
          )}
          {clientInfo?.email && (
            <Text style={styles.subtitle}>{clientInfo.email}</Text>
          )}
        </View>
        <View style={styles.table}>
          <View style={styles.tableRow}>
            ...
            <View style={{ ...styles.tableColHeader, width: "11%" }}>
              <Text style={styles.tableCellHeader}>Debit (Rs. )</Text>
            </View>
            <View style={{ ...styles.tableColHeader, width: "11%" }}>
              <Text style={styles.tableCellHeader}>Credit (Rs. )</Text>
            </View>
            <View style={{ ...styles.tableColHeader, width: "12%" }}>
              <Text style={styles.tableCellHeader}>Balance (Rs. )</Text>
            </View>
          </View>
          {ledgerData.map((row, i) => (
            <View style={styles.tableRow} key={i}>
              ...
              <View style={{ ...styles.tableCol, width: "12%" }}>
                <Text style={styles.tableCell}>{row.reference || "-"}</Text>
              </View>
        ...
        >
          ...
          >
            <Text
              style={{ fontSize: 10, fontWeight: "bold", color: "#4B5563" }}
            >
              Total Billed
            </Text>
            <Text
              style={{ fontSize: 11, fontWeight: "bold", color: "#111827" }}
            >
          ...
          >
            ...
            <Text
              style={{ fontSize: 11, fontWeight: "bold", color: "#059669" }}
            >
          ...
          >
            ...
            <Text
              style={{ fontSize: 12, fontWeight: "bold", color: "#E11D48" }}
            >
...
const ClientLedger = () => {
    ...
    try {
      if (exportFormat === "excel") {
        ...
        const blob = await pdf(
          <LedgerPDF clientInfo={clientInfo} ledgerData={ledgerData} />,
        ).toBlob();
  ...
  return (
    <div className="max-w-7xl print-container" style={{ padding: "32px 0px" }}>
      {/* Header */}
      <div
      ...
      >
        <div className="flex items-center" style={{ gap: "16px", minWidth: 0 }}>
          <button
            ...
            className="bg-white border border-gray-200 hover:bg-gray-50 rounded-xl transition-all shadow-sm flex items-center justify-center flex-shrink-0"
            style={{
              padding: "8px",
              width: "40px",
              height: "40px",
          ...
          <div className="min-w-0 flex-1">
            <h1
              className="text-3xl font-semibold text-gray-900 tracking-tight leading-tight truncate"
              title={clientInfo ? clientInfo.name : "Client Ledger"}
              style={{ maxWidth: "100%" }}
            >
              {clientInfo ? clientInfo.name : "Client Ledger"}
            </h1>
            <p
              className="text-base text-gray-500 truncate"
              style={{ marginTop: "4px" }}
              title={
                clientInfo
                  ? `${clientInfo.phone || "No Phone"}${clientInfo.panNumber ? ` • PAN: ${clientInfo.panNumber}` : ""}`
                  : "Loading details..."
              }
            >
              {clientInfo ? (
        ...
        <button
          ...
          className="flex items-center bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 flex-shrink-0"
          style={{
            padding: "8px 16px",
            gap: "8px",
            height: "40px",
      ...
      >
        ...
        >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <p
              className="text-2xl lg:text-3xl font-bold text-gray-900 tracking-tight print:text-xl"
              style={{
                maxWidth: "100%",
                wordBreak: "break-word",
                overflowWrap: "anywhere",
        ...
        >
          ...
          >
            <Wallet
              className={`w-6 h-6 ${currentBalance > 0 ? "text-rose-600" : "text-gray-600"}`}
            />
          </div>
          <div style={{ minWidth: 0, flex: 1 }}>
            <p
            ...
            <p
              className={`text-2xl lg:text-3xl font-bold tracking-tight print:text-xl ${
                currentBalance > 0 ? "text-rose-600" : "text-gray-900"
              }`}
              style={{
                maxWidth: "100%",
                wordBreak: "break-word",
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-auto">
        <div>
          <AppleDataTable
            ...
            loading={loading}
            rowKey={(row) =>
              (row.paymentId || row.invoiceId) + row.type + row.date
            }
            defaultSortKey="date"
            defaultSortDir="asc"
