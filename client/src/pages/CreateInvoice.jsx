...
const CreateInvoice = () => {
  ...
  const convertTimeNotationToDecimal = (timeNotation) => {
    ...
    let mins = parseInt(minutes, 10) || 0;
  ...
  // Helper: Convert decimal hours to numeric HH.MM format (for display)
    ...
    items: [
      {
        ...
        description: "",
        hsnCode: "",
        displayQuantity: 1, // For UI display (HH.MM for time, number for others)
        quantityDecimal: 1, // Decimal value for calculations and backend storage
        unitType: "item",
        pricingType: "fixed",
        baseRate: 0,
        pricingTiers: [],
  ...
  const handleTimeChange = (index, type, value) => {
    ...
    // Format as HH.MM for display (as number, but will be formatted as string when needed)
      ...
      items: prev.items.map((item, i) =>
        i === index
          ? {
              ...
              displayQuantity: displayQuantityNumber,
            }
          : item,
      ),
    }));
  ...
  const handleServiceChange = (index, serviceId) => {
    ...
    if (selectedService) {
      ...
      let displayQuantity = initialQuantity;
      let timeHours = 1;
      let timeMinutes = 0;

      if (isTimeUnit) {
        // Convert initial quantity from HH.MM to decimal
        quantityDecimal = convertTimeNotationToDecimal(initialQuantity);
        // Keep display quantity as formatted number
        displayQuantity = convertDecimalToTimeNotation(quantityDecimal);
        const { hours, minutes } =
  ...
  const cleanPayload = (payload) => {
    ...
    return {
      ...
      items: payload.items.map((item) => {
        ...
        if (isTimeUnit) {
          // For time units: send quantity (number for legacy), quantityDisplay (string with 2-digit minutes), quantityDecimal (decimal for calculations)
          const formattedDisplayQuantity = formatTimeNotation(
            item.quantityDecimal,
          );
          cleanedItem.quantity = convertDecimalToTimeNotation(
            item.quantityDecimal,
          ); // Legacy number format (e.g., 4.3)
          cleanedItem.quantityDisplay = formattedDisplayQuantity; // Display string (e.g., "4.30")
          ...
          >
            ...
            >
              {formData.items.map((item, index) => {
                ...
                // Format display quantity for time units to show with 2-digit minutes
                const formattedDisplayQty =
                  isTimeUnit && item.quantityDecimal
                        ...
                        >
                          ...
                          >
                            ...
                            {isCollapsed && isTimeUnit && (
                              ...
                              >
                                Qty: {formattedDisplayQty} {item.unitType}
                              </div>
                            )}
                          </div>
