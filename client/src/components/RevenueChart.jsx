...
const RevenueChart = ({ data, dateRange = 12, viewType = "all" }) => {
  const chartData = useMemo(() => {
    ...
    for (let i = dateRange - 1; i >= 0; i--) {
      ...
      const monthYearStr = d.toLocaleString("default", {
        month: "short",
        year: "numeric",
      });
    ...
    data.forEach((inv) => {
      ...
      const key = `${invDate.getFullYear()}-${String(
        invDate.getMonth()
      ).padStart(2, "0")}`;
  ...
  const scalingInfo = useMemo(() => {
    ...
    return {
      needsScaling: maxPending > 0 && maxTotal / maxPending > 10,
    };
  }, [chartData]);

  // IMPORTANT FIX
  // Dynamically calculate enough width for long Y-axis labels
  const yAxisWidth = useMemo(() => {
    const maxValue = Math.max(
      ...chartData.map((d) =>
        Math.max(d.total, d.collected, d.pending)
      ),
      0
    );

    const formatted = `Rs. ${(maxValue / 1000).toFixed(0)}k`;

    // Approximate width calculation
    return Math.max(80, formatted.length * 8);
  }, [chartData]);
  ...
  const renderCustomLegend = () => {
    const items = [
      ...
      if (viewType === "bar" && item.key !== "total") {
        return false;
      }

      if (viewType === "line" && item.key === "total") {
        return false;
      }
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      ...
      >
        ...
        >
          {!inactiveSeries.collected &&
            (viewType === "all" || viewType === "line") && (
              <div
                style={{
                  display: "flex",
          ...
          {!inactiveSeries.total &&
            (viewType === "all" || viewType === "bar") && (
              <div
                style={{
                  display: "flex",
                  justifyContent: "space-between",
              ...
              >
                ...
                <span style={{ fontVariantNumeric: "tabular-nums" }}>
                  Rs. {d.total.toLocaleString("en-IN")}
                </span>
              </div>
            )}
        </div>
      ...
      >
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart
            ...
            margin={{
  top: 10,
  right: scalingInfo.needsScaling ? 40 : 30,
  left: 30,
  bottom: 10,
}}
          >
