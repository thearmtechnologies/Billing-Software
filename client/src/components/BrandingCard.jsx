...
const BrandingCard = ({ currentLogoUrl, onLogoChange }) => {
  ...
  useEffect(() => {
    if (currentLogoUrl) setPreviewUrl(currentLogoUrl);
  }, [currentLogoUrl]);
  ...
  const processFile = (file) => {
    ...
    // Fix: Remove process.env which causes Vite to crash silently
    ...
    // Resize if needed (max width 600px)
    const img = new Image();
  ...
  const uploadFile = async (file) => {
    ...
    try {
      setUploading(true);
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/users/upload-logo`, formData, {
  ...
  const handleRemove = async () => {
    try {
      await axios.delete(`${import.meta.env.VITE_BASE_URL}/users/remove-logo`, { withCredentials: true });
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200" style={{ padding: '24px', marginBottom: '24px' }}>
      <h3 className="text-lg font-semibold text-gray-900" style={{ marginBottom: '16px' }}>Company Logo</h3>
      <div
      ...
      >
        {previewUrl ? (
          <img src={previewUrl} alt="Company logo preview" className="max-h-32 object-contain" style={{ marginBottom: '8px' }} />
        ) : (
          <Upload className="w-8 h-8 text-gray-500" style={{ marginBottom: '8px' }} />
        )}
        <p className="text-sm text-gray-600">{previewUrl ? 'Click to replace logo' : 'Drag & drop or click to upload logo'}</p>
        <input
          type="file"
      ...
      {previewUrl && (
        ...
        >
          <Trash2 className="w-4 h-4" style={{ marginRight: '4px' }} /> Remove Logo
        </button>
      )}
    </div>
  );
